...
const VoiceAssistant: React.FC<VoiceAssistantProps> = ({ deviceType }) => {
  ...
  // Temporary transcription accumulation
    ...
    try {
      ...
      const sessionPromise = ai.live.connect({
        ...
        callbacks: {
          onopen: () => {
            ...
            scriptProcessor.onaudioprocess = (audioProcessingEvent) => {
              ...
              // CRITICAL: Rely on sessionPromise resolves
          ...
          onmessage: async (message: LiveServerMessage) => {
            ...
            if (message.serverContent?.outputTranscription) {
              ...
              updateTranscriptionUI('ai', currentOutputRef.current);
            } else if (message.serverContent?.inputTranscription) {
              currentInputRef.current += message.serverContent.inputTranscription.text;
              updateTranscriptionUI('user', currentInputRef.current);
            }
        ...
        config: {
          ...
          systemInstruction: `You are a world-class Cisco Network Expert. You are currently assisting with a ${deviceType}. 
          Keep your verbal answers concise, technically accurate, and professional. 
          You must respond primarily in ${selectedLang.name}. 
          If you are explaining complex configuration, speak clearly.`,
  ...
  const updateTranscriptionUI = (type: 'user' | 'ai', text: string) => {
    setTranscriptions(prev => {
      const last = prev[prev.length - 1];
      // If the last message is of the same type, we update it instead of adding a new one
      // to create a smooth "streaming" text effect in the UI
      if (last && last.type === type && (Date.now() - last.timestamp.getTime() < 5000)) {
        const updated = [...prev];
        updated[updated.length - 1] = { ...last, text, timestamp: new Date() };
        return updated;
      }
        ...
        {transcriptions.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-slate-500 text-center space-y-3">
            <div className="w-16 h-16 bg-slate-900/50 rounded-full flex items-center justify-center border border-slate-800">
              <i className="fas fa-comment-dots text-2xl opacity-20"></i>
            </div>
            <p className="text-sm">Start a conversation in {selectedLang.name}.<br/><span className="text-[10px] opacity-60">Ask about routing, switching, or security.</span></p>
          </div>
        ) : (
          transcriptions.map((t, i) => (
            <div key={i} className={`flex ${t.type === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-1 duration-300`}>
